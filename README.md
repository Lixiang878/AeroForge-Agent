...
经典风洞烟线构图：种子位置按几何包围盒自动推导，速度着色默认采用感知均匀的
Viridis 色表（Cividis 作为兼容回退）；色标上下限按当前有效流线样本动态确定并保留
...
`|U| (m/s)` 色标、播放/时间滑块、前视/尾流/俯视预设，并支持鼠标轨道旋转和滚轮缩放；
...
- **可视化只画真实场**：pvpython 自动发现（PATH/常见目录）→ 模板脚本
  ...
  车辆采用可配置统一车漆，Viridis（Cividis 回退）色标避免遮挡与彩虹色误读；速度跨度较大时使用
