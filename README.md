...
**当前验证状态（如实声明）**：八轮可追溯实验（见 [`docs/VALIDATION.md`](docs/VALIDATION.md)）
显示设置级修正（圆角、阻塞比、入口湍流、地面滑移、尾流加密）把 Cd 从尖角的
1.056 一路拉到 0.49，之后进入平台期；剩余偏差由稳态 RANS/壁面函数模型形态
与无支腿简化几何主导，达 ±10% 容差需低雷诺近壁大网格或 DES/LES，已列入
改进路线。所有数值均为真实求解输出，含中途发现并修复的 wakeBox 语法静默
失效等三个真实 bug 的完整记录。诚实边界见下文「设计边界」。
