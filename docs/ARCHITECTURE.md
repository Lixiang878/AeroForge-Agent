...
- `interactive_viz.py` 调用 `export_streamline_data.py` 从 ParaView 获取实际 `U`、
  ...
  Plotly 使用车辆聚焦的手动纵横比、轨道相机、输运时间滑块和物理速度色标（蓝—青—黄—红与 log10 规则）。
  页面可在连续流线、速度采样点云、尾流截面和组合视图之间切换；点云/截面均只取已导出的
  真实 `U` 采样，不凭空插值压力或湍流量。HTML 是冻结稳态 RANS 的无质量示踪展示，
  不与 P7 等未对齐视觉模型混用。稳态输运窗按 `8L/U∞`，清单明确记录其与 solver
  迭代时间不同的语义。
