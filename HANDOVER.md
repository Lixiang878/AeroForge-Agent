...
- `workspace/` 继续作为 E 盘本地产物区并整体被 `.gitignore` 排除；当前保留项、历史审计
  证据、可再生缓存和重新生成规则见 [`docs/WORKSPACE_LAYOUT.md`](docs/WORKSPACE_LAYOUT.md)。
- 已将根目录 `.pytest_cache/`、各级 `__pycache__/`/`*.pyc`、未引用的重复
  `drivaernet_official` 克隆、已复制到 `docs/media/` 的重复动画媒体，以及失败 showcase 的
  无效网格二进制移入 Windows 回收站；报告/日志/字典和当前求解证据仍在 E 盘保留。
- 清理前对正式 HTML/GIF/MP4 做 SHA-256 对照，未删除 `docs/` 正式交付物；`workspace/_scratch/`
  当前为空，仅保留父目录以满足 `pytest.ini` 的 `--basetemp`。

## 0.9. 交互 XYZ 等单位显示比例修正（2026-08-31）

- `streamline_data.json` 中车体顶点和速度采样仍使用同一米制坐标；用户看到的车身偏高是
  Plotly 显示比例问题，不是 STL 或 CFD 几何被缩放。
- 旧版按车体长宽高设置 `aspectmode=manual`，但显示范围还包含长尾流，使 Z 每米的屏幕尺度
  约为 X 的 `1.5549` 倍。现在比例按实际显示轴跨度计算，run_1/refined 与 run_2 均复核
  `z/x=1.0000`，保留车体聚焦的默认取景和完整可缩放数据。
- 已重新生成 `docs/media/drivaerml_steady_particles.html` 与
  `docs/media/drivaerml_run_2_steady_particles.html`；该修正不改 STL、求解场或静态投影。
