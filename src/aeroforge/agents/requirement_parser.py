...
from ..core.models import SimulationTask, FlowType, Regime, GeometrySource
_STRIP_WORDS = ("做|一[辆个款只]|仿真|模拟|测试|计算|CFD|风洞|稳态|瞬态|不可压|工况|可视化|流线|图|一下|的|进行|情况|条件下|给|我|请|帮忙|风速|风向")
# 车型/物体关键词：命中即作为对象名核心（覆盖常见真实诉求）
_OBJ_KW = (r"(?:Ahmed(?:\s*body)?|NACA\s?\d{3,4}|简化车(?:辆)?|圆柱绕流|球绕流|圆柱|圆球|球体|方块|方柱|翼型"
           r"|宝马|奔驰|奥迪|大众|丰田|本田|日产|特斯拉|Tesla|比亚迪|蔚来|小鹏|问界|极氪"
           r"|保时捷|Porsche|福特|雪佛兰|凯迪拉克|沃尔沃|红旗|长安|吉利|长城|奇瑞|理想|car|cylinder|sphere)")
...
def _extract_object_name(text: str) -> str:
    ...
    km = re.search(rf"{_OBJ_KW}", text)
    if km:
        name = km.group(0).strip()
        suffix = re.match(r"([A-Za-z0-9][A-Za-z0-9_.-]*)", text[km.end():])
        spaced_suffix = re.match(r"\s+([A-Za-z0-9][A-Za-z0-9_.-]*)", text[km.end():])
        model = suffix or spaced_suffix
        if model and re.search(r"[\u4e00-\u9fff]", name):
            name += model.group(1)  # 中文关键词后跟型号：'宝马 X3' -> '宝马X3'
...
class RequirementParserAgent:
    ...
    async def run(self, prompt, **kwargs):
        ...
        yaw=0.0
        ym=(re.search(r"(?:风向角|迎风角|迎风|偏航(?:角)?|攻角|yaw)\s*(?:为|:|=)?\s*([-+]?[0-9]+(?:\.[0-9]+)?)\s*(?:°|度|deg)",text,re.I)
        ...
        return {"status":"completed","task":SimulationTask(user_prompt=text,object_name=object_name,velocity=velocity,
                                                            ...
                                                            geometry_source=GeometrySource.UPLOAD if upload else GeometrySource.PARAMETRIC,
